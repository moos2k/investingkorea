      ...
      >
        <div className="p-4 border-b border-gray-200">
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-base font-bold text-gray-900">종목 추가</h2>
          ...
          <input
            ...
            className="w-full bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:border-blue-400"
          />
          <div className="text-xs text-gray-400 mt-2">
            💡 한국 종목은 <span className="text-gray-600">005930.KS</span>(코스피) / <span className="text-gray-600">.KQ</span>(코스닥) 형식으로 검색하세요.
          </div>
        </div>

        <div className="max-h-80 overflow-y-auto">
          {loading && <div className="p-4 text-sm text-gray-400 text-center">검색 중...</div>}
          {!loading && query && results.length === 0 && (
            <div className="p-4 text-sm text-gray-400 text-center">검색 결과가 없습니다</div>
          )}
          {!loading &&
            results.map((r) => (
              <button
              ...
              >
                <div>
                  ...
                  <div className="text-xs text-gray-400">{r.symbol} {r.exchange ? `· ${r.exchange}` : ""}</div>
                </div>
                <span className="text-xs text-gray-500 bg-gray-100 rounded px-2 py-0.5">{r.type}</span>
              </button>
            ))}
        </div>
      </div>
